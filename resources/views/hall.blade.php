...
@section('content')
    ...
    <div class="max-w-4xl mx-auto mb-18">
        ...
        <div class="text-center mt-4">
            <h3 class="text-2xl font-bold"><a class="text-gray-900 hover:text-blue-500" href="/hall/book{{ $books[0]->slug }}">{{ $books[0]->name }}</a></h3>
            <div class="flex justify-center items-center text-gray-600 text-sm gap-4 mt-2">
                <span class="flex items-center gap-1"><i class="fa-solid fa-user text-blue-600"></i> <a href="/hall/book{{ $books[0]->author->slug }}" class="hover:text-blue-500">{{ $books[0]->author->name }}</a></span>
                <span class="flex items-center gap-1"><i class="fa-solid fa-bookmark text-green-500"></i> <a href="#" class="hover:text-blue-500">{{ $books[0]->category->name }}</a></span>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            @foreach ($books as $book)
                <div class="bg-white shadow-md rounded-lg overflow-hidden">
                    <div class="relative">
                        @if ($book->cover)
                            <img src="{{ Storage::url($book->cover) }}" alt="Cover Buku">
                        @else
                            <img src="https://picsum.photos/1200/400" alt="Cover Buku" class="w-full h-96 object-cover">
                        @endif
                    ...
                    <div class="p-4">
                        <h5 class="text-lg font-bold"><a href="/hall/book/{{ $book->slug }}" class="hover:text-blue-500">{{ $book->name }}</a></h5>
                        ...
                        <a href="/hall/book/{{ $book->slug }}" class="mt-4 inline-block px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Read more</a>
                    </div>
                </div>
            @endforeach 
        </div>
